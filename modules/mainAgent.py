...
llm = ChatOpenAI(api_key=OPENAI_API_KEY,model="gpt-4o-mini", temperature=0.5,max_tokens=550,timeout=25)
...
REGLAS DE RESPUESTA:
- Sé breve: máximo 2-3 oraciones y 120 palabras.
...
- Nunca copies ni pegues la salida de las tools.
- Usa las tools solo como insumo: interpreta los datos, selecciona lo más relevante y reescríbelo con tus palabras.
- Si la tool devuelve varios resultados, muestra solo los 2-3 más útiles para un turista.
...
- Cierra con una recomendación útil o una frase breve de orientación turística, no con contexto técnico.
...
def create_workflow(user_id: str, user_name: str = '', lang: str = 'es', client_source: str = 'avatarCDT' )-> StateGraph:
    ...
    def agent_node(state: State):
        ...
        print(f" mensaje del agente: ")
        print(result["messages"][-1].content)
        print(f"fin mensaje del agente: ")
